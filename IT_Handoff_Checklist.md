...
- [ ] **Desktop shortcut** — this is a web app, not an installer, so there's no icon on the desktop by default. Push a `.url` shortcut (Windows) pointed at `https://residna.internal.yourcompany.com` via whatever your company already uses for desktop provisioning (GPO, Intune, etc.), the same way other internal web tools get a desktop icon. Ask for this explicitly if employees expect one — it's not automatic, and IT has no way to know it's wanted otherwise.
